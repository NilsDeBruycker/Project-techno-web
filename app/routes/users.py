...
@router.post("/sign_up")
def sign_up_route(username: Annotated[str, Form()],email:Annotated[str, Form()],password: Annotated[str, Form()],password2: Annotated[str, Form()],seller : Annotated[bool,Form()]=False):
    if user_service.get_user_by_email(email) is not None:
        ...
        user_service.sign_up_user(new_user)
...
def promote_user_route(email:Annotated[str, Form()], current_user: UserSchema = Depends(login_manager)):
    ...
    user_service.promote_user(email)
    return RedirectResponse(url="/users/", status_code=302)
...
def modify_monney(new_username:Annotated[str, Form()], current_user: UserSchema = Depends(login_manager)):
    user_service.modify_monney(new_username,current_user)
    return RedirectResponse(url="/users/profile", status_code=302)"""
