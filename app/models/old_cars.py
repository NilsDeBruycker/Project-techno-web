...
class User(Base):
    ...
    seller      : Boolean[str]= mapped_column(Boolean)


class Client(Base):
    __tablename__ = 'clients'
    id : Mapped[int] = mapped_column(Integer, ForeignKey('User.email'), primary_key=True)
...
class Vehicle(Base):
    ...
    sold  :Mapped[Boolean] = mapped_column(Boolean)# = relationship("Sale", back_populates="vehicle")
    price_sell :Mapped[Float] = mapped_column(Float)
    price_rent :Mapped[Float] = mapped_column(Float)
    #ligne de rel avec rent
    rented :Mapped[Boolean] = mapped_column(Boolean)
    ...
    owner_email: Mapped[int] = mapped_column(ForeignKey("User.email"),nullable=True)
